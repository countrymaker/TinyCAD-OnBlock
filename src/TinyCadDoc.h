...
public:
	...
	BOOL CanUndo() const
	{
		return m_undo_level > 0;
	}
	
	BOOL CanRedo() const
	{
		return m_undo_level + 1 < static_cast<int> (m_undo.size());
	}
	...
	void SetNameDir(int i)
	{
		NameDir = i;
	}
	
	void SetPinDir(int i)
	{
		PinDir = i;
	}
	...
	BOOL GetSnapToGrid()
	{
		return m_snap.GetGridSnap();
	}
	
	void SetSnapToGrid(BOOL r)
	{
		m_snap.SetGridSnap(r);
	}
