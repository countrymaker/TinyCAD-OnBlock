...
//
BOOL CFolderFrame::Create(CWnd* pParent, CRuntimeClass* pViewClass, CCreateContext* pcc, UINT nIDRes, int cxFolderTabCtrl, DWORD dwStyle)
{
	ASSERT(pParent);
	...
	CRect rc(0, 0, 0, 0);
	if (!CreateEx(0, lpClassName, NULL, dwStyle, rc, pParent, AFX_IDW_PANE_FIRST)) return FALSE;

	CWnd* pView = (CWnd*) pViewClass->CreateObject();
	ASSERT(pView);
	if (!pView->Create(NULL, NULL, dwStyle, rc, this, AFX_IDW_PANE_FIRST, pcc)) return FALSE;
	...
	if (nmw.code == NMWINMGR::GET_SIZEINFO)
	{
		switch (wp)
		{
			case AFX_IDW_HSCROLL_FIRST:
				nmw.sizeinfo.szMin = CSize(MINSCROLLBARWIDTH, GetSystemMetrics(SM_CYHSCROLL));
				return TRUE;
			case AFX_IDW_VSCROLL_FIRST:
				nmw.sizeinfo.szMin.cx = GetSystemMetrics(SM_CXVSCROLL);
				return TRUE;
			case ID_WIN_SIZEBOX:
				nmw.sizeinfo.szMin = CSize(GetSystemMetrics(SM_CXVSCROLL), GetSystemMetrics(SM_CYHSCROLL));
				return TRUE;
...
{
	if (width == bestFit) width = m_wndFolderTabCtrl.GetDesiredWidth();
...
{
	return nBar == SB_HORZ ? const_cast<CScrollBar*> (&m_wndSBHorz) : nBar == SB_VERT ? const_cast<CScrollBar*> (&m_wndSBVert) : NULL;
}
...
{
	GetView()->SendMessage(WM_HSCROLL, MAKELONG(nSBCode, nPos), (LPARAM) pScrollBar->m_hWnd);
}
void CFolderFrame::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	GetView()->SendMessage(WM_VSCROLL, MAKELONG(nSBCode, nPos), (LPARAM) pScrollBar->m_hWnd);
}
...
{
	if (GetView()->OnCmdMsg(nID, nCode, pExtra, pHandlerInfo)) return TRUE;
