...
/*
 ...
 TinyCAD sources are modified by countrymaker 2022.
...
void CNetList::WriteNetListFile(int type, CTinyCadMultiDoc *pDesign, const TCHAR *filename)
{
	switch (type)
	{
		...
		//< cmaker
		case 7:
			WriteNetListFileKiCad(pDesign, filename);
			break;
		case 8:
			WriteNetListFileOnBlock(pDesign, filename);
			break;
		//> cmaker
...
void CNetList::WriteNetListFileKiCad(CTinyCadMultiDoc* pDesign, const TCHAR* filename) {
	std::ofstream outfile;
	outfile.open(filename);
	...
	auto tmp = make_unique<COnBlock>();
	auto onblock = move(tmp);

	const string xmlfilename = CT2A(filename);
	const size_t size = onblock->loadXmlFile(xmlfilename);
	if (size == 0) return;

	onblock->parse1();
	onblock->parse2();

	string dsnname = onblock->getDsnName();
	if (dsnname.empty()) {
		MessageBox(NULL, L"No design name", L"OnBlock", 0);
		return;
	}

	make_dir(onblock->getDsnFolderName());
	ofstream nfile(onblock->getNetFileName());
	const string rstr = onblock->generateKicadNetlist();
	nfile.write(rstr.c_str(), rstr.length());
	nfile.close();

	const CString csfilename = onblock->getNetFileName().c_str();
	CTinyCadApp::EditTextFile(csfilename);
}

void CNetList::WriteNetListFileOnBlock(CTinyCadMultiDoc* pDesign, const TCHAR* filename) {
	std::ofstream outfile;
	outfile.open(filename);
	...
	const string dsnname = onblock->getDsnName();
	if (dsnname.empty()) {
		MessageBox(NULL, L"No design name", L"OnBlock", 0);
		return;
	}
	...
	make_dir(onblock->getDsnFolderName());
	ofstream inofile(onblock->getInoFileName());
	const string rstr = onblock->composeInoFile(COnBlock::compose_option::exclude_unconnected_parts);
	inofile.write(rstr.c_str(), rstr.length());
	inofile.close();

	const CString csfilename = onblock->getInoFileName().c_str();
	CTinyCadApp::EditTextFile(csfilename);
}
